...
shinyServer(function(input, output) {
  selected_player = reactive({
    season_stats %>% filter(., Player == input$selected_player) %>% group_by(Player) %>% summarise(
      Position = Pos,
      Season = n(),
  ...
  era_mean = reactive({
    season_stats %>% filter(.,
                            Year >= input$Year[1] &
                              Year <= input$Year[2] &
                              Pos == input$selected_player$Pos)
    %>% arrange(., desc(PER)) %>% distinct(., Player, .keep_all = T) %>% slice_head(., n = 20)
    %>% group_by(Pos)
    %>% summarise(
      Player = "Era",
      Position = Pos,
  ...
  avg_player_mean = reactive({
    season_stats %>% filter(.,
                            Year >= input$Year[1] &
                              Year <= input$Year[2] &
                              Pos == input$selected_player$Pos) %>% group_by(Pos) %>% summarise(
                                Player = "Average",
                                Position = Pos,
                                Season = n(),
                                Mean_G = mean(G, na.rm = T),
  ...
  output$stat_compare = renderPlot({
    combined_df %>% ggplot(aes(x = Player, y = input$comparison)) + geom_col(fill =
                                                                               "lightblue")
  })
